...
class SpectFormer(nn.Module):
    def __init__(self, dim, depth, heads, dim_head, mlp_dim, h, w, att_dropout = 0., ff_dropout = 0., drop_path = 0., alpha = 4, use_flash = True):
        super().__init__()
        ...
        for i in range(depth):
            if i < alpha:
                ...
                self.layers.append(
                    ...
                    PreNorm(dim, FeedForward(dim, mlp_dim, dropout = ff_dropout))),
                )
    def forward(self, x):
...
class ViTFlashAttnSpect(nn.Module):
    def __init__(self, *, image_size, patch_size, num_classes, dim, depth, heads, mlp_dim, channels = 3, dim_head = 64, att_dropout = 0., ff_dropout = 0., drop_path = 0., spect_alpha = 4, use_flash = True):
        super().__init__()
        ...
        self.transformer = SpectFormer(dim, depth, heads, dim_head, mlp_dim, h, w, att_dropout, ff_dropout, drop_path, spect_alpha, use_flash)
